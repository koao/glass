...
- **シーケンス図** — 選択したプロトコル電文をUML風のシーケンス図として可視化
- **チェックサム / CRC 検証** — フレームのチェックサム（CRC-16 / SUM / XOR）を検証し、不一致をハイライト
- **プロトコル検索** — プロトコルパネル内をタイトル・フィールド内容で検索
- **パターントリガ** — 指定バイトパターンを受信したら自動で一時停止
- **選択 & コピー** — モニタ・プロトコル画面で範囲選択し、Ctrl+Cまたは右クリックメニューでコピー（ASCII / HEX / バイナリ形式）
- **配色カスタマイズ** — モニタ画面の文字色を設定から変更可能
...
選択した電文をUML風のシーケンス図として描画します。送信元・宛先からパーティシパントを自動生成し、要求/応答の流れを視覚的に追えます。

![シーケンス図](docs/screenshots/sequence_diagram_ja.png)

### TOML形式

全キー・フレームルール・チェックサム指定・シーケンス図式の網羅的なリファレンスは [docs/protocol_format.ja.md](docs/protocol_format.ja.md) を参照してください。
